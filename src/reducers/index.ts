...
function calc(formulas: Formula[], operator: Action.Operator, num: number) {
  const result =
    formulas.length === 0 ? 0 : formulas[formulas.length - 1].result;
  switch (operator) {
    case "+":
      return result + num;
    case "-":
    ...
    case Action.APPEND_ROW: {
      return {
        formulas: [
          ...
          {
            ...state.formula,
            //id: state.formulas[state.formulas.length - 1].id + 1
            id:
